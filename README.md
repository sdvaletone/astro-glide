...
- **"Auto-pull failed: You have uncommitted local changes"**: Profilarr’s local clone of this repo has edits (e.g. to `profiles/1080p-Anime.yml` or other files) that block `git pull`. To get updates from this repo: in Profilarr, **commit** your local changes (Database → stage and commit), then pull; or **revert** the modified file(s) to discard local edits, then pull. If you don’t need to keep local edits, reverting and then pulling brings your copy in line with this repo (including quality `id`s and profile updates).
